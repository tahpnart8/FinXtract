...
async def extract_pdf(
    ...
    logger.info(f"Job {job_id} started in background for {ticker}/{period}")
...
async def generate_excel_endpoint(request: GenerateExcelRequest):
    ...
    """
    try:
        # Lấy kỳ bắt đầu và kết thúc để log
        p_from = request.periods[0] if request.periods else "Unknown"
        p_to = request.periods[-1] if request.periods else "Unknown"
        logger.info(f"Generating Excel for {request.ticker} ({p_from}-{p_to})")

        # Convert yearly_data into the nested dict format required by excel_writer
        # Format: {"income_statement": { "revenue": { "2022-Q1": 100, "2023": 120 } }, ...}
        mapped_data = {
            "income_statement": {},
            "balance_sheet": {},
            "cash_flow": {}
        ...
        for y_data in request.yearly_data:
            ...
            for key, val in data.items():
                ...
                for section in mapped_data:
                    ...
                    mapped_data[section][key][period] = val
